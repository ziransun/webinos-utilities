...
function Config(webinosType, inputConfig) {
    ...
    */
    function setWebinosMetaData() {
        ...
        try {
	    //This is the place where deviceStatus API is called
            require("./webinosId.js").fetchWebinosName(webinosType, inputConfig, function (webinosName) {
            	if (process.platform == 'android') {
					try {
		    			...
		    			var notification = bridge.load('org.webinos.android.impl.PZPNotificationManagerImpl', exports);
		    			notification.eventNotify("progress", function(status){
		        			console.log("device status - send notification on PZP status:" + status);
				    	});
					}	
					catch(e) {
		    			console.log("Android pzp notification - error: "+e.message);
					}
				} 
        ...
        certList.forEach(function(name){
            if (existsSync(path.join(ConfigContext.metaData.webinosRoot, name.folderName, name.fileName+".json"))){
                ...
                if (name.folderName = "certificates/internal") {
                    ConfigContext.fetchDetails("certificates/internal", "signedCertificates", ConfigContext.cert.internal.signedCert);
                }
